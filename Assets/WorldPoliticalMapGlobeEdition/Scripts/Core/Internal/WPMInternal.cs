...
namespace WPM {
    ...
    public partial class WorldMapGlobe : MonoBehaviour {
        ...
        void OnEnableDelayed() {
            ...
            // Remove legacy collider and rigidbody
        ...
        void LateUpdate() {
            ...
            if (_frontiersThicknessMode == FRONTIERS_THICKNESS.Custom) {
                Shader.SetGlobalMatrix("_CustomObjectToWorld", transform.localToWorldMatrix);
            }
        }
        ...
        float GetMinEarthRadius() { return transform.lossyScale.x * 0.5f + _zoomMinDistance + MIN_ZOOM_DISTANCE; }
        ...
        protected virtual void CheckUserInteractionNormalMode() {
            ...
            if (!pinchZooming && (mouseIsOver || hasDragged || rightMouseButtonPressed)) {
                ...
                if (_allowUserRotation) {
                    ...
                    if (capturePosition) {
                        ...
                        if (_dragConstantSpeed) {
                            ...
                            dragDirection *= 0.01f * distFactor * _mouseDragSensitivity * Time.deltaTime * 60f;
#endif
                        ...
                        if (orbitDirection.x != 0 || orbitDirection.y != 0) {
                            ...
                            if (!lockYaw) {
                                yaw += direction * orbitDirection.x * _mouseDragSensitivity * 0.01f * Time.deltaTime * 60f;
                            }
                            if (!lockPitch) {
                                pitch += direction * orbitDirection.y * _mouseDragSensitivity * 0.01f * Time.deltaTime * 60f;
                            }
                        }
                        StopAnyNavigation();
                    }
            ...
            if (dragDampingStart > 0 && !leftMouseButtonPressed && !rightMouseButtonPressed) {
            ...
            if (zoomDistance != 0) {
                PerformZoomInOut(cam, zoomDistance);
            }
        ...
        protected virtual void CheckUserInteractionInvertedMode() {
                ...
                if (_allowUserRotation) {
                    if (leftMouseButtonClick) {
                        ...
                        if (_dragConstantSpeed) {
                            ...
                            dragDirection.x = ApplyDragThreshold(dragDirection.x, _mouseDragThreshold);
                            dragDirection.y = ApplyDragThreshold(dragDirection.y, _mouseDragThreshold);
                            dragDirection *= 0.015f * distFactor * _mouseDragSensitivity * Time.deltaTime * 60f;
#endif
            ...
            if (wheelAccel != 0) {
                ...
                if (wheelAccel >= 0.01f || wheelAccel <= -0.01f) {
                    cam.fieldOfView = Mathf.Clamp(cam.fieldOfView + (90.0f * cam.fieldOfView / MAX_FIELD_OF_VIEW) * wheelAccel * _mouseWheelSensitivity * Time.deltaTime * 60f, MIN_FIELD_OF_VIEW, MAX_FIELD_OF_VIEW);
                    if (_dragConstantSpeed) {
                        wheelAccel = 0;
                    } else {
        ...
        void RotateAround(Transform transform, Vector3 center, Vector3 axis, float angle) {
            ...
            //transform.rotation *= Quaternion.Inverse(myRot) * rot * myRot; // TODO: RML
